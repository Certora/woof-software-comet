...
import { ethers, expect, exp, makeProtocol, presentValue, ZERO_ADDRESS, presentValueSupply, mulPrice, mulFactor, defaultAssets, MAX_ASSETS, baseBalanceOf, event, portfolio, setTotalsBasic, wait, fastForward, UserCollateral, UserBasic } from './helpers';
...
describe('transfer', function () {
  ...
  before(async () => {
    ...
    // Additional setup for AUDIT tests
    ...
    const collateralsForMaxAssets = Object.fromEntries(
      Array.from({ length: MAX_ASSETS }, (_, j) => [`ASSET${j}`, { initialPrice: 100, decimals: 18 }])
    );
    const protocolWithMaxAssets = await makeProtocol({
      assets: { USDC: {}, ...collateralsForMaxAssets },
    });
    cometWithExtendedAssetListMaxAssets = protocolWithMaxAssets.cometWithExtendedAssetList;
